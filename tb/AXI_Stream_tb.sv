...
module AXI_Stream_tb;
        ...
        if (!reset) begin
            ...
            if (prev_m_tvalid && !prev_m_tready) begin
                ...
                if (m_tdata !== prev_m_tdata) begin
                    $display(
                        "[%0t] *** PROTOCOL FAIL: m_tdata changed while stalled: %0d -> %0d ***",
    ...
    task automatic flush_pipeline(int max_cycles);
        ...
        s_tvalid = 0;
        if (scoreboard.size() != 0) begin
            $display("*** FAIL: flush timed out with %0d beats never emerged ***",
                     scoreboard.size());
            error_count++;
        end
    endtask
    ...
    initial begin
        ...
        $display("=================================================================");
        $display(" AXI_Stream_tb: %0d samples, LATENCY=%0d, S_WIDTH=%0d, M_WIDTH=%0d", NUM_SAMPLES,
                 LATENCY, S_DATA_WIDTH, M_DATA_WIDTH);
        $display("=================================================================");
